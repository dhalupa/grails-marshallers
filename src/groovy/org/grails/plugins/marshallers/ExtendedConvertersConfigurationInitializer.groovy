...
class ExtendedConvertersConfigurationInitializer implements ApplicationContextAware {
    ...
    private Map collectMarshallingConfigsFromDomainClasses() {
        ...
        application.domainClasses.each {
            ...
            if (mc) {
                MarshallingConfig c = evaluateConfig(clazz, new Stack())
                ['xml', 'json'].each { type ->
                    def map = nc[type]
                    if (map == null) {
                        map = [:]
    ...
    private MarshallingConfig evaluateConfig(Class clazz, Stack configStack) {
        if (clazz != Object) {
            Closure mc = GCU.getStaticPropertyValue(clazz, 'marshalling')
            if (mc) {
                ...
                configStack.push(c)
                return evaluateConfig(clazz.superclass, configStack)
            }
        }
        MarshallingConfig config = null
        while (!configStack.empty()) {
            config = config ? config.mergeWith(configStack.pop()) : configStack.pop()
        }
        return config
