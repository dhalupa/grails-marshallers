...
class ConfigurationBuilder {
    ...
    def registerSpringMarshallers() {
        ...
        applicationContext.grailsApplication."${type}MarshallerClasses"*.clazz.findAll { clazz ->
            def cfg = GrailsClassUtils.getStaticPropertyValue(clazz, CONFIGURATION_FIELD)
    ...
    def invokeMethod(String name, args) {
        if (args.size() != 1) {
            ...
            def builder = new ConfigurationBuilder(parentConf: parentConf, type: type, applicationContext: applicationContext, cfg: newCfg, converterClass: converterClass, cfgName: name)
