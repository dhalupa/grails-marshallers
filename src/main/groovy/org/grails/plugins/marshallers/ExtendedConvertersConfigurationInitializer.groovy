...
class ExtendedConvertersConfigurationInitializer implements ApplicationContextAware {
    ...
    private Map collectMarshallingConfigsFromDomainClasses() {
        ...
        application.domainClasses.each {
            def clazz = it.clazz
            Closure mc = GrailsClassUtils.getStaticPropertyValue(clazz, 'marshalling')
            if (mc) {
    ...
    private def processConfig(cfg, Class converterClass, type) {
        ...
        def parentConf = [:]
        def builder = new ConfigurationBuilder(parentConf: parentConf, type: type, applicationContext: applicationContext, cfg: converterCfg, converterClass: converterClass, cfgName: DEFAULT_CONFIGURATION_NAME)
